...
export const Answer = ({ answer, onCitationClicked, onExectResultClicked }: Props) => {
  ...
  const createCitationFilepath = (citation: Citation, index: number, truncate: boolean = false) => {
    ...
    if (citation.title) {
      const part_i = citation.partIndex ?? (citation.chunkId ? parseInt(citation.chunkId) + 1 : '')
    ...
    } else if (citation.title && citation.reindexId) {
      citationFilename = `${citation.title} - Part ${citation.reindexId}`
    } else {
  ...
  const onLikeResponseClicked = async () => {
    if (answer.message_id == undefined || CONVERSATION_ID == undefined) return
    ...
    await historyMessageFeedback(CONVERSATION_ID, answer.message_id, newFeedbackState)
  }
  ...
  const onDislikeResponseClicked = async () => {
    ...
    if (feedbackState === undefined || feedbackState === Feedback.Neutral || feedbackState === Feedback.Positive) {
      ...
      newFeedbackState = Feedback.Neutral
      setFeedbackState(newFeedbackState)
      await historyMessageFeedback(CONVERSATION_ID, answer.message_id, Feedback.Neutral)
    }
    appStateContext?.dispatch({
      type: 'SET_FEEDBACK_STATE',
      payload: { answerId: answer.message_id, feedback: newFeedbackState }
  ...
  const onSubmitNegativeFeedback = async () => {
    if (answer.message_id == undefined || CONVERSATION_ID == undefined) return
    await historyMessageFeedback(CONVERSATION_ID, answer.message_id, negativeFeedbackList.join(','))
    resetFeedbackDialog()
  }
